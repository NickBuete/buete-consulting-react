...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const status =
      statusParam && (Object.values(HmrReviewStatus) as string[]).includes(statusParam)
        ? (statusParam as HmrReviewStatus)
        : undefined;

    const patientId = parseId(typeof req.query.patientId === 'string' ? req.query.patientId : undefined);
    const clinicId = parseId(typeof req.query.clinicId === 'string' ? req.query.clinicId : undefined);
    const prescriberId = parseId(typeof req.query.prescriberId === 'string' ? req.query.prescriberId : undefined);
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const symptom =
      symptomKey && (SymptomType as Record<string, SymptomType>)[symptomKey as keyof typeof SymptomType];
